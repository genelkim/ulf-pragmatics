...
;
 (cond ((eq x y) t)
       ((null (get x 'isa)) nil)
       ((eq (get x 'isa) x) nil); cannot reach y
       (t (isa (get x 'isa) y)) ))
...
;; TODO: handle conjoined vp heads (e.g. ((past stop.v) and.cc (past think.v))
;; TODO: put this in ulf-lib
(defun remove-vp-tense! (vp)
;````````````````````````````
; Removes the tense from the given VP.
; NB: this does not remove all tenses. Rather it removes the tense for this
; paricular verb phrase, retaining the tense of embedded tensed clauses.
  (let ((vhead (ulf:find-vp-head vp)))
    (if (and (listp vhead)
             (= (length vhead) 2)
             (ulf:lex-tense? (first vhead)))
      (replace-vp-head vp (second vhead))
      vp)))

(defun implicit-lex-prep? (x) (and (lex-prep? x) (lex-elided? x)))
(defun implicit-lex-det? (x) (and (lex-det? x) (lex-elided? x)))
...
; function for matching verbs or auxiliaries
  (or (ulf:lex-verbaux? sym) (aux-like? sym)))
...
;; If there are nested verb arguments, flatten them into canonical form.
(defparameter *ttt-flatten-verb-arg*
  '(/ (((!1 lex-verb? (lex-tense? lex-verb?)) _+) term?)
      (!1 _+ term?)))
(defun flatten-verb-args (ulf)
  (ttt:apply-rule *ttt-flatten-verb-arg* ulf :max-n 1000))
